...
int main() {
  ...
  auto test1_rangev3 = [&] {
    ...
    ret = ranges::accumulate(rng1 | filter(is_even) | transform(x3), 0);
  };
  ...
  auto test3_rangev3 = [&] {
    ...
    ret = ranges::accumulate(rng3 | unique | filter(is_even), 0);
  };
  ...
  auto test5_rangev3 = [&] {
    ...
    ret = ranges::accumulate(rng5 | transform(rangev3_unique_adaptor) | join |
                                 filter(is_even) | transform(x3),
                             0);
  };
  ...
  ankerl::nanobench::render(ankerl::nanobench::templates::csv(), bench,
                            std::cout);
}
