...
struct is_compatible<RangeAdaptor, Range,
                     std::void_t<decltype(std::declval<RangeAdaptor>()(
                         std::declval<Range &>()))>> : std::true_type {};

template <typename RangeAdaptor, typename Range>
constexpr auto is_compatible_v = is_compatible<RangeAdaptor, Range>::value;

template <typename RangeAdaptor> auto equivalent_transranger(RangeAdaptor ra) {
...
bool same_output(Range &rng, RangeAdaptor ra, Transranger tr) {
  auto v = ra(rng);
  using view_value_type =
      std::remove_cv_t<std::remove_reference_t<decltype(*std::begin(v))>>;
  std::vector<view_value_type> res1;
  for (const auto &x : v)
    res1.push_back(x);

  auto rgr = tr(transrangers::all(rng));
  using ranger_value_type = std::remove_cv_t<std::remove_reference_t<
